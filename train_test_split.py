...
def train_val_test_set(label_file,Gene_file,TF_file,train_set_file,val_set_file,test_set_file,density,p_val=args.p_val):
    ...
    gene_set = pd.read_csv(Gene_file)['index'].values # 靶基因序号
    tf_set = pd.read_csv(TF_file)['index'].values # tf序号
    ...
    val_pos = {}
    test_pos = {}
    ## 训练集测试集的划分都是从每个tf对应的靶基因里面按照一定比例划分开
...
## 负采样
    ...
    for k in train_pos.keys():
        train_neg[k] = []
        for i in range(int(len(train_pos[k]))):
...
if __name__ == '__main__':
    ...
    train_set_file = 'processed_data/'+data_type+'/Train_set.csv'
    test_set_file = 'processed_data/'+data_type+'/Test_set.csv'
    val_set_file = 'processed_data/'+data_type+'/Validation_set.csv'
